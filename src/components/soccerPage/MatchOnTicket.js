...
const MatchOnTicket = ({ info: { id, game, oddValue } }) => {
  // context
  const { matchesToShow, deleteFromTicket } = useContext(SoccerContext);

  // state for played match
  ...
  const getInfo = () => {
    matchesToShow.forEach((match) => {
      match.id === id && setPlayedMatch(match);
    });
  };
  ...
  return (
    <div className="w-full mb-1 p-2 border-[1px] border-blue-500 text-white">
      ...
      <div className="w-full mb-1 border-b-[1px] border-b-white flex items-center justify-between">
        <h1>{playedMatch && playedMatch.commence_time.slice(0, 10)}</h1>
        <FaTrash
      ...
      <div className="w-full flex items-center justify-between">
        <div className="w-[60%] flex flex-col text-sm">
          <h1>{playedMatch && playedMatch.home_team}</h1>
          <h1>{playedMatch && playedMatch.away_team}</h1>
        </div>
